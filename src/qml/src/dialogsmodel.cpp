...
static Vreen::Contact *getContact(const Vreen::Message &message, const Vreen::Client *const client)
{
	return client->contact(message.isIncoming() ? message.fromId()
												: message.toId());
}
...
void DialogsModel::onAddMessage(const Vreen::Message &message)
{
	...
	for (int i = 0; i != count(); i++) {
		auto old = at(i);
		if (getContact(message, m_client.data()) == getContact(old, m_client.data())) {
